...
This program provides an interactive data visualization and analysis tool for exploring the tallest skyscrapers
in the United States. It loads and processes a dataset containing information about skyscrapers, including their
locations, heights, materials, and construction years. Users can navigate through different sections of the app
to view a scatter map of skyscrapers, bar graphs comparing city data, pie charts showing material distributions,
and line graphs depicting construction trends over time. The program also highlights interesting facts, such as
the tallest and shortest buildings, the most used construction material, and the city with the most skyscrapers.
"""
...
#Load all of the data and read from the folder
def load_data():
    try:
        data = pd.read_csv('skyscrapers (1).csv')
        return data
    except FileNotFoundError:
        st.error(
            "File 'skyscrapers (1).csv' not found in the current directory. Please make sure it's placed correctly.")
